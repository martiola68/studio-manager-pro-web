...
async function context(request: Request) {
  ...
  const { data: user, error: userError } = await db.from("tbutenti").select("studio_id,tipo_utente,attivo").ilike("email", email).maybeSingle();
...
export async function GET(request: Request) {
  try {
    ...
    const { data: studio, error: studioError } = await db.from("tbstudio").select("ragione_sociale,licenze_bypass,stato_cancellazione,data_richiesta_cancellazione,data_cancellazione_programmata,data_fine_servizio").eq("id", studioId).maybeSingle();
    if (studioError) throw new Error("STUDIO_QUERY");
    ...
    const deletionStatus = studio.stato_cancellazione || null;
    ...
    return NextResponse.json({
      ...
      deletion_status: deletionStatus, deletion_requested_at: studio.data_richiesta_cancellazione || null, deletion_scheduled_at: studio.data_cancellazione_programmata || null,
      service_ended_at: studio.data_fine_servizio || null, can_request_deletion: !deletionScheduled, can_cancel_deletion: deletionScheduled,
    });
  } catch (error: any) {
